...
- [`useAccount`](/apis/modules/__orderly_network___hooks/useAccount)：获取当前账户信息及状态，并且可以通过该 hook 获取[`Account`](/apis/modules/__orderly_network___hooks/Account)单例。
- [`useCollateral`](/apis/modules/__orderly_network___hooks/useCollateral) - 获取当前账户的余额
- [`useLeverage`](/apis/modules/__orderly_network___hooks/useLeverage) - 获取当前杠杆倍数
...
- `NotConnected`/ `Connected` - 钱包连接状态，与 Orderly API 无关；
- `NotSignedIn` / `SignedIn` - 调用 Orderly API [`Get Account`](https://docs-api-evm.orderly.network/#restful-api-public-get-account) 或者 [`Register Account`](https://docs-api-evm.orderly.network/#restful-api-public-register-account)获取到的结果;
- `DisabledTrading` / `EnableTrading` - 调用 Orderly API [`Get Orderly Key`](https://docs-api-evm.orderly.network/#restful-api-public-get-orderly-key) 或者 [`Add Orderly Key`](https://docs-api-evm.orderly.network/#restful-api-public-add-orderly-key)获取到的结果;
