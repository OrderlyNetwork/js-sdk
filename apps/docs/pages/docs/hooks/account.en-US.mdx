...
- [`useAccount`](/apis/modules/__orderly_network___hooks/useAccount): Retrieves the current account info and status. You can also use this hook to get the ['Account'](/apis/modules/__orderly_network___hooks/Account) singleton.
...
- Subscribe to the `change:status` event in the 'Account' instance. (`account` is a singleton of the `Account` instance: more information can be found [here](/apis/modules/__orderly_network___hooks/Account))。
