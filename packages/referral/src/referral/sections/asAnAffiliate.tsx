...
export const AsAnAffiliate = () => {
  ...
  const bottomInfo = useMemo(() => {
    ...
    if (isAffiliate) {
      return (
        <div className="orderly-mt-3 orderly-text-[24px] lg:orderly-txt-[26px] 2xl:orderly-text-[30[px] orderly-flex orderly-justify-between orderly-items-end">
          <div>
            <div className="orderly-text-xs md:orderly-text-base 2xl:orderly-text-[18px] orderly-text-base-contrast-80">{tr("referral.ref.card.comission")}</div>
    ...
    return (
      <div className="orderly-flex orderly-justify-between md:orderly-mt-2 orderly-items-center">
        ...
        <div>
          <div className="orderly-text-[22px] md:orderly-text-[24px] lg:orderly-text-[26px] xl:orderly-text-[26px] 2xl:orderly-text-[28px]">{tr("referral.not.ref.card.hint.title")}</div>
          <div className="orderly-text-3xs 2xl:orderly-text-xs orderly-text-right orderly-text-base-contrast-54">{tr("referral.not.ref.card.hint.subtitle")}</div>
        </div>
      </div>
    );

  }, [referralInfo?.referrer_info?.total_referrer_rebate, isAffiliate, overwrite, tr]);
  ...
  const icon = useMemo(() => {
    ...
    if (typeof overwrite?.ref?.card === 'object' && (overwrite?.ref?.card as OverwiteCard)?.refIcon !== undefined) {
  ...
  const children = useMemo(() => {
    if (typeof overwrite?.ref?.card === 'object' &&
      ...
      return ((overwrite?.ref?.card as OverwiteCard).ref as Function)(state);
    }

    return (
      <div
        id="dashboard_affiliate_container"
        className={cn("orderly-rounded-xl orderly-w-full orderly-p-6 orderly-bg-gradient-to-t orderly-from-referral-bg-from orderly-to-referral-bg-to orderly-h-[195px] lg:orderly-h-[199px] xl:orderly-h-[216px] 2xl:orderly-h-[248px] orderly-flex orderly-flex-col orderly-justify-between", className)}
      >
