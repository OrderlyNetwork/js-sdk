...
*/
export const useMarketsList = (type: MarketListType) => {
  // TODO: rename to orderly_markets
  ...
  const getData = (type: MarketListType) => {
    // get data
    const localData =
      type === MarketListType.FAVORITES ? [...favorites] : [...recent];
    // filter
    const keys = localData.map((item) => item.name);
    const filter =
      type == MarketListType.ALL
        ? data
