...
export function RenderWalletIcon({ connector }: { connector: Connector | WalletAdapter }) {
  const icon = connector.icon ? connector.icon : getWalletIcon((connector as Connector).type)
  return (
    <div
      className="oui-w-[18px] oui-h-[18px] oui-flex oui-items-center oui-justify-center"
    >
      <img
        className={cn(
