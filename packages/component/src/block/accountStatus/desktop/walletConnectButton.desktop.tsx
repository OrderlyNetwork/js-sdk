...
> = (props) => {
    ...
    if (typeof value === "object") {
      return (
        ...
        (typeof value === "string" ||
          ...
          (Array.isArray(value) &&
            value.every((item) => typeof item.$$typeof === "symbol")))
      );
    }
  ...
  return (
    ...
    >
      ...
      <DropdownMenuContent
        align="end"
        className="orderly-bg-base-800 orderly-w-[220px] orderly-p-4 orderly-rounded-borderRadius orderly-shadow-[0px_12px_20px_0px_rgba(0,0,0,0.25)] orderly-z-20"
        alignOffset={10}
      ...
      >
        <div className="orderly-flex">
          <div className="orderly-flex-1 orderly-flex orderly-items-center orderly-gap-2">
            ...
            <Text
              className="orderly-text-xs orderly-text-base-contrast-80"
              rule={"address"}
            >
              {account.address}
            </Text>
            ...
            <Tooltip>
              <TooltipTrigger>
                <IconButton
                  type="button"
                  onClick={onCopy}
                  className="orderly-px-0 orderly-ml-2 orderly-w-[20px]"
                >
                  {/*@ts-ignore*/}
                  <Copy
                    size={20}
                    className="orderly-stroke-base-contrast-54 hover:orderly-stroke-base-contrast"
                  />
                </IconButton>
              </TooltipTrigger>
              <TooltipContent
            ...
            <Tooltip>
              <TooltipTrigger>
                <IconButton
                  ...
                  className="orderly-px-0 orderly-w-[20px]"
                >
                  {/*@ts-ignore*/}
                  <Share
                    size={20}
        ...
        {dropMenuItem && Array.isArray(dropMenuItem) && (
          <Divider className="orderly-pb-2" />
        )}
        {dropMenuItem &&
          Array.isArray(dropMenuItem) &&
          dropMenuItem.map((item) => {
            return (
              ...
              >
                <span className="orderly-text-base-contrast-36 hover:orderly-text-base-contrast orderly-text-xs">
                  {item.title}
                </span>
              </button>
            );
        ...
        {/*@ts-ignore*/}
        {dropMenuItem && isReactNode(dropMenuItem) && { dropMenuItem }}

        <Divider className="orderly-my-2" />
        <div
          className="orderly-flex orderly-cursor-pointer orderly-items-center orderly-w-full orderly-h-[24px] orderly-fill-base-contrast-36 orderly-text-base-contrast-36 hover:orderly-fill-danger-light hover:orderly-text-danger-light"
          onClick={() => {
            setOpen(false);
            onWalletDisconnect?.();
          }}
