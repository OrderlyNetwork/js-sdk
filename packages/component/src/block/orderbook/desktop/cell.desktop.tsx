...
export const DesktopOrderBookCell: FC<DesktopOrderBookCellProps> = (props) => {
  ...
  const isPendingOrder = useMemo(() => {
    ...
    const index = pendingOrders.findIndex(
      (item) => priceStr === parseNumber(item, { precision: dp, padding: true })
    );
  ...
  return (
    <div
      className="orderly-flex orderly-flex-row orderly-tabular-nums orderly-justify-between orderly-text-base-contrast-80 orderly-text-3xs orderly-relative orderly-cursor-pointer"
    ...
    >
      <div
        className={cn(
          "orderly-basis-7/12 orderly-flex orderly-felx-row orderly-items-center orderly-mr-2",
      ...
      >
        ...
        <div className="orderly-flex-1 orderly-text-right orderly-text-base-contrast-80">
          <Numeral precision={base_dp}>{props.quantity}</Numeral>
        </div>
      </div>
      <div
        className={cn(
      ...
      >
        ...
        >
          <Numeral precision={base_dp} className="orderly-z-10">
            {props.accumulated}
          </Numeral>
        </div>
        {showTotal && (
          <div className="orderly-flex-1 orderly-text-right orderly-pr-3">
            <Numeral precision={2} className="orderly-z-10">
              {totalAmount}
            </Numeral>
          </div>
        )}
      ...
      {props.isHover && (
        <div className="orderly-absolute orderly-bg-white orderly-left-0 orderly-right-0 orderly-top-0 orderly-bottom-0 orderly-opacity-10"></div>
      )}
    </div>
  );
};
