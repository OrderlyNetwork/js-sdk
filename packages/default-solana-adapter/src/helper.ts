...
import {
    ...
    getSendLibInfoPda,
    getSendLibPda,
    getTokenPDA, getUlnEventAuthorityPda,
    getUlnSettingPda,
    getUSDCAccounts,
...
import {
    ...
    SEND_LIB_PROGRAM_ID, TREASURY_PROGRAM_ID
} from "./constant";
...
export async function getDepositQuoteFee({
    ...
    const quoteParams = {
        dstEid: DST_EID,
        to: Array.from(getPeerAddress(appProgramId)),
        options: Buffer.from([]),
        message: lzMessage,
        payInLzToken: false
    };
    ...
    console.log(decodedBuffer.readBigUInt64LE(0), decodedBuffer.readBigUInt64LE(1));
