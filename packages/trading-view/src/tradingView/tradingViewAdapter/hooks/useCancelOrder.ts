...
export default function useCancelOrder() {
    const [pendingOrders, { cancelOrder, cancelAlgoOrder, cancelTPSLChildOrder }] = useOrderStream({
    ...
    return useCallback((order: any) => {
        if (order.algo_order_id) {
            if (TpslAlgoType.includes(order.root_algo_order_algo_type)) {
                ...
                const rootOrder = pendingOrders?.find(item => item.algo_order_id === order.root_algo_order_id)
                const isEditActivated = rootOrder.child_orders.every((item: any)=> !!item.trigger_price)
                if (isEditActivated) {
                    return cancelTPSLChildOrder(order.algo_order_id, order.root_algo_order_id);
                }
