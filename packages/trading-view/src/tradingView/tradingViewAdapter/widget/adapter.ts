...
const persistAdapterSettings = (adapterKey: string, key: string, value: any) => {
    const settings = loadAdapterSettings(adapterKey);
    localStorage.setItem(adapterKey, JSON.stringify({ ...settings, [key]: value }));
};
...
export const getSettingAdapter = (adapterKey: string) => {
    ...
    return {
        ...
        setValue: (key: string, value: any) => persistAdapterSettings(adapterKey, key, value),
        removeValue: () => {},
    };
};
