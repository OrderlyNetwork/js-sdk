import { IBrokerConnectionAdapterHost, IChartingLibraryWidget } from "../charting_library";
...
function forceSilentOrdersPlacement  (instance: IChartingLibraryWidget, host:IBrokerConnectionAdapterHost) {
    instance.onChartReady(() => {
        ...
        host.silentOrdersPlacement().subscribe((val: any) => {
            if (!val) {
                ...
                if (instance) {
                    (instance as any)._iFrame.contentDocument.querySelector('.wrapper-3X2QgaDd').className =
...
export default function brokerHostHandler(instance: IChartingLibraryWidget, host:IBrokerConnectionAdapterHost){
