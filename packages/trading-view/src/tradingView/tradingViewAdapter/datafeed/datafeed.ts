...
import {EXCHANGE, withExchangePrefix, withoutExchangePrefix} from '../util';
...
export class Datafeed extends AbstractDatafeed {
    ...
    public getQuotes(symbols: string[], onDataCallback: QuotesCallback): void {
        ...
        const unsub = this.eventBus.subscribe('tickerUpdate', (msg: any) => {
            ...
            onDataCallback(Array.from(dataMap.values()).map((symbolData: any) => this._toUDFTicker(symbolData)));
    ...
    public subscribeQuotes(symbols: string[], fastSymbols: string[], onRealtimeCallback: QuotesCallback, listenerGuid: string): void {
        ...
        if (symbols.length > 0) {
            ...
            const unsub = this.eventBus.subscribe('tickerUpdate', (msg: any) => {
                ...
                onRealtimeCallback(Array.from(dataMap.values()).map((symbolData: any) => this._toUDFTicker(symbolData)));
            });
            this._subscribeQuoteMap.set(subscriptionId, unsub);
        }
    }
