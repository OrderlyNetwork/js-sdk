...
import { RequestParams, UdfErrorResponse, UdfOkResponse, UdfResponse, getErrorMessage } from './helpers';
...
export class HistoryProvider {
    ...
    public getBars(
        ...
        return new Promise(async (resolve: (result: GetBarsResult) => void, reject: (reason: string) => void) => {
            try {
                const initialResponse = await this._requester.sendRequest<HistoryResponse>(this._datafeedUrl, 'history', requestParams);
    ...
    private async _processTruncatedResponse(result: GetBarsResult, requestParams: RequestParams) {
        ...
        try {
            while (
                ...
                const followupResponse = await this._requester.sendRequest<HistoryResponse>(this._datafeedUrl, 'history', requestParams);
